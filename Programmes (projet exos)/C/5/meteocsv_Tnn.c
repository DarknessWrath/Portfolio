//  affiche le minimum des températures minimales négatives (Tn)
...
//  min_tnn : Cherche le minimum des températures négative
...
int max_tn(const char *filename, float *ptr) {
  ...
  //    && n == (Minimum (tn) de température négative, sans compter le dernier
  //              relevé lu si rfrs == 0)
