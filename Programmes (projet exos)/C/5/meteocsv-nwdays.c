//  affiche le nombre de jours d'ensoleillement
...
int nb_w(const char *filename, int *ptr);

//--- mesure et unité

#define MEASURE_TXT "Le nombre de jours d'ensoleillement"
#define UNIT_TXT    "jours"
...
int nb_w(const char *filename, int *ptr) {
  ...
  //    && n == (le nombre de relevés lus avec w>0 des relevés lus,
  //       sans compter le dernier relevé lu si rfrs == 0)
